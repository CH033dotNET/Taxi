	...
	public class CarModelValidator : AbstractValidator<CarDTO>
	{
		public CarModelValidator()
		{
			RuleSet("AddNewCar", () => {
				...
				RuleFor(xcar => xcar.CarName).NotEmpty().Length(2, 20).Matches(@"^[A-Z]{1}[a-z]{1,10}[ ]{0,1}([A-Z]{1}[a-z]{2,10}){0,1}").WithLocalizedMessage(() => Resources.Resource.CarNameError); // validation rule for {CarName} property
